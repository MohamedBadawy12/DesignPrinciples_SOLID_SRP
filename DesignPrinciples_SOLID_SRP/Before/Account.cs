namespace DesignPrinciples_SOLID_SRP.Before
{
    internal class Account
    {
        ...
        public void MakeTransaction(decimal amount)
        {
            ...
            if (amount < 0)
            {
                ...
                {
                    transactionMessage = $"OVERDRAFT when trying to withdraw " +
                ...
                {
                    this.Balance = amount;
                    transactionMessage = $"OK Withdraw {Math.Abs(amount).ToString("C2")}" +
                       $", current balance {Balance.ToString("C2")}";
                }
            }
            else
